...
So all `n` points equal `p(s+k)`. Not "usually", not "for most seeds" — **always**, for every seed and every `num_points`. Measured:
...
Two consequences. Both position sets are drawn from a generator built from the *same* integer — and the clobber means **a caller cannot work around it**: pass `voters_positions_args={"seed": 1}` and it is discarded on the next line. There is no user-side fix; only a patch.
...
So the "escape" is not protection. It is a stride mismatch, and it evaporates the moment `num_voters == num_candidates`:
...
Worth noting that a language with move/borrow semantics makes this class of bug hard to write at all. Rust's `rand` takes `rng: &mut impl Rng` — a *mutable borrow*, so two samplers cannot hold the same generator simultaneously, and forking a stream (`SeedSequence::spawn`'s analogue, `SeedableRng::from_rng`) is an explicit, visible call. A `u64` seed is `Copy`, so `f(seed); g(seed);` compiles silently in Rust exactly as it does in Python — the protection comes from the *idiom of passing the generator*, which the borrow checker then enforces. Same rule, enforced by the type system instead of by review.
